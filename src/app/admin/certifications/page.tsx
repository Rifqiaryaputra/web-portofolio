...
export default function AdminCertificationsPage() {
  ...
  function handleSaved() {
    setToast(editing ? "Certification updated." : "Certification added.");
    refresh();
  }

  async function handleDelete(item: Certification) {
    if (
      !confirm("Are you sure you want to delete this certification entry?")
    )
      return;
    try {
              ...
              <tbody>
                {items.map((item, i) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-title max-w-[200px] truncate">
                      {item.url ? (
                        <a
                          href={item.url}
                          target="_blank"
